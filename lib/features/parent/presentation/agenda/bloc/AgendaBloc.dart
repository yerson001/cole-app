...
class AgendaBloc extends Bloc<AgendaEvent, AgendaState> {
  ...
  AgendaBloc(this.parentUseCases)
    : super(AgendaState(selectedDate: DateTime.now())) {
    on<LoadAgenda>((event, emit) async {
      ...
      if (event.studentId != null && event.studentId! > 0) {
        ...
        if (result is SuccessResource<AgendaModel>) {
          emit(
            state.copyWith(
              items: _agendaOnly(result.data.items),
              isLoading: false,
              clearError: true,
            ),
          );
  ...
  List<AgendaItemModel> _agendaOnly(List<AgendaItemModel> items) =>
